...
function sendInputToGAS3(message, isUserMessage) {
    const GAS_URL3 = 'https://script.google.com/macros/s/AKfycbwAR5iFfX0lXnf-gROuIYqroqxgCP_X_N9M-YtfpRYwGUEdx-F725dXHDrvsU1mtYIicQ/exec'; // Replace with your actual URL
...
function convertTextToHtml(text) {
    ...
    // Convert "BOOK A CALL WITH THE OWNER" into a clickable link
    replacedText = replacedText.replace(/BOOK A CALL WITH THE CEO/g, function() {
        return '<a href="https://calendly.com/jaiwaterman/15min?back=1&month=2024-05" target="_blank">BOOK A CALL WITH THE CEO</a>';
    });
...
function startConversation() {
    fetch('https://chatbot-for-blockstation-2-samuelsicking.replit.app/start', {
        method: 'GET'
    })
    .then(response => response.json())
    .then(data => {
...
document.getElementById('chat-form').addEventListener('submit', function(event) {
    ...
    fetch('https://chatbot-for-blockstation-2-samuelsicking.replit.app/chat', {
        method: 'POST',
        headers: {
...
function startNewConversation() {
    ...
    setTimeout(function() {
        ...
        displayChatbotMessage("Hi there, what about blockstation interests you?");
    }, 2000);
}
...
function sendToGAS(message, isUserMessage) {
    var GAS_URL = 'https://script.google.com/macros/s/AKfycbyMuHkYWlEp5A9Am81hlcoMr653701tYsbT6wDRZlNH-tvi4BTSYepJjkCD05COjMHp4Q/exec';
...
function sendClickDataToGAS() {
    const url = 'https://script.google.com/macros/s/AKfycbzSqIF47Bc1KrNM3bguWBdPgNjXn0ePNtYVhXWGeCxjjooFeXlTy8MpzoWv5K1EN5hcdQ/exec'; // Replace YOUR_SCRIPT_ID with your actual deployed script ID
    fetch(url, {
        method: 'POST',
...
function sendToGas4(message, isUserMessage) {
    ...
    const GAS_URL4 = 'https://script.google.com/macros/s/AKfycbxghbhJw6lT36_SxPbvaAspbHeVaEcBy9EVXPe-DULZWIinyfJwveU3MpxvCfVWk1Ro/exec';
